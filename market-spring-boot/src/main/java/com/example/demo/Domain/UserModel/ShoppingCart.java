...
public class ShoppingCart {
    ...
    public void resetCart() {
        List<DataShoppingBag> datas = new ArrayList<>();
        shoppingBagHash.forEach((i, sb) -> {
            datas.add(sb.getDataObject());
        });
        if (dataServices != null && dataServices.getShoppingBagService() != null) {
            if (!dataServices.getShoppingBagService().deleteShoppingBags(datas, username)) {
                logger.error(String.format("failed to delete shopping bags of user %s", username));
            }
        }
        shoppingBagHash.forEach((i, sb) -> {
            sb.reset();
        });
        shoppingBagHash.clear();
    }
