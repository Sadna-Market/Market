...
//        grantee.notifyAll();
    ...
    public DResponseObj<Boolean> removeOwnerPermissionCompletely(User grantee, Store store, User grantor) {
        ...
        if (store.getFounder().value.equals(grantee.getEmail().value)) {
...
/*        //change all permission grantor to the founder (because we delete the grantor user)
        for (Permission permission : grantee.getGrantorPermission().value) {
            Store s = permission.getStore().value;
            String founder = s.getFounder().value;
            if (!founder.equals(grantee.getEmail().value)) {
                User founderUser = getMember(founder).value;
                permission.setGrantor(founderUser);
                founderUser.addGrantorPermission(permission);
            }
        }*/
        return removeOwnerRecursive(grantee,store);
    }
...
//*********************************************** private method ******************************************************
    ...
    private Permission getPermission(User grantee, Store store, User grantor) {
        ...
        List<Permission> accessPermissionStore = store.getPermission().value;
        for (Permission p : accessPermissionStore) {
            if (p.getGrantor().value != null && p.getGrantee().value.getEmail().value.equals(grantee.getEmail().value) && p.getGrantor().value.getEmail().value.equals(grantor.getEmail().value)) {
                return p;
            }
        }
        return null;
