...
public class RuleService {
   ...
   // @Transactional(rollbackFor = {Exception.class}, timeout = 10)
    ...
    // @Transactional(rollbackFor = {Exception.class}, timeout = 10)
    public boolean insertBuyRule(DataBuyRule buyRule,int storedID) {
        try {
            Optional<DataStore> dataStore = storeRepository.findById(storedID);
            if(dataStore.isEmpty()){
                logger.warn(String.format("store %d is not present in db", storedID));
                return false;
            }
